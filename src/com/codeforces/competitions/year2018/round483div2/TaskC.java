	...
	static class Solver implements Runnable
	{
		...
		void solve() throws IOException
		{
			...
			while (n-- > 0)
			{
				...
				String pb = convFrmDecToBase(p, b);
				String qb = convFrmDecToBase(q, b);
				BigInteger pp = new BigInteger(pb);
				BigInteger qq = new BigInteger(qb);
				BigInteger base = BigInteger.valueOf(b);

				for (int i = 1; i <= 100; i++)
				{
					if (pp.mod(qq).equals(BigInteger.ZERO))
					{
						out.println("Finite");
...
//					pp = pp.multiply(base);
		...
		static String convFrmDecToBase(long num, long base)
		{
