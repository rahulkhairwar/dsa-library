    ...
    static class Solver
    {
		...
		void solve()
		{
			...
			for (int i = 1; i < n; i++)
			{
				...
				if (nodes[u].colour == nodes[v].colour)
					union(nodes[u], nodes[v]);
			}

			Set<Integer> white, black;
			...
			out.println(Math.min(white.size(), black.size()));
		}
		...
		class Node
		{
			int key, colour, height, size;
			Node parent;

			public Node(int key, int colour)
			{
				this.key = key;
				this.colour = colour;
				height = 0;
