...
public class TaskD
{
	...
	static class Solver implements Runnable
	{
		...
		void solve() throws IOException
		{
			...
			for (int i = n - 2; i >= 0; i--)
			{
				System.out.println("i : " + i + ", arr[i] : " + arr[i] + ", ans[i + 1] : " + ans[i + 1]);

				if (arr[i] > 0)
					ans[i] = nCR(n - i - 1, (int) arr[i]);

				if (ans[i + 1] > 0)	// can take this element or not take it.
					ans[i] = CMath.mod(ans[i] + CMath.mod(ans[i + 1] /** 2*/, MOD), MOD);
