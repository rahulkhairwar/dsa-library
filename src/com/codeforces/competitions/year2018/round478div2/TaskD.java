...
//		BufferedReader in;
		...
		void solve() throws IOException
		{
			...
			for (int i = 0; i < n; i++)
			{
				double slope = slope(x[i], y[i], x2[i], y2[i]);
				Long cnt = map.get(slope);
		...
		double slope(double x, double y, double m, double n)
		{
			System.out.println("x : " + x + ", y : " + y + ", m : " + m + ", n : " + n);

			if (m == x)
				return (n - y > 0 ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY);
			return (n - y) / (m - x);
		}
