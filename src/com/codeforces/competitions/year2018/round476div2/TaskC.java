...
//		BufferedReader in;
		...
		void solve() throws  IOException
		{
			...
			{
				long parts = 1 + (i - 1) * k;
				long eachPartQuantity = n / parts;

//				System.out.println("parts : " + parts + ", ePQ : " + eachPartQuantity);

				if (eachPartQuantity > max)
				{
					ans = Math.max(ans, (1 + parts / k) * max);

					continue;
				}

				ans = Math.max(ans, eachPartQuantity * i);
			}
...
//			System.out.println("candies : " + candies);
			...
			tot = Math.min(tot, d);
//			System.out.println("candies : " + candies + ", tot : " + tot);

			if (tot * candies < maxPossible)
				return false;

			if (tot >= 1)
				maxPossible = tot * candies;
