...
//		BufferedReader in;
		...
		void solve() throws IOException
		{
			...
			for (int i = 0; i < n; i++)
			{
				System.out.println("i : " + i + ", av : " + available + ", cnt[i] : " + cnt[i]);
				if (cnt[i] > i + 1 || (i < n - 1 && cnt[i] > available + 1))
				{
					out.println("NO");
...
/*			cum[0] = cnt[0];
			...
			for (int i = 0; i < n; i++)
			{
				System.out.println("i : " + i + ", cnt[i] : " + cnt[i] + ", cum[i] : " + cum[i]);
				if (cnt[i] > i + 1 || (i < n - 1 && cnt[i] > 0 && (cnt[i] - (i > 0 ? cum[i - 1] : 0) <= 0)))
				{
					out.println("NO");
