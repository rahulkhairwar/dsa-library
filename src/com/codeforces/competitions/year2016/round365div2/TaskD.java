	...
	static class Solver
	{
		...
		void solve()
		{
			...
			for (int i = 1; i <= n; i++)
			{
				...
				// element has already occured in the array before
				if (pos[curr] != -1)
					add(pos[curr], arr[i]);
				...
				{
					ans[queries[counter].index] =
							query(i) ^ query(queries[counter].left - 1) ^ cumXor[i] ^ cumXor[queries[counter].left - 1];
					counter++;
				}
			}
