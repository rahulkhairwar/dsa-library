...
//			System.out.println("gr : " + Arrays.toString(groups));
		...
		boolean isValid(List<Long> list, long x)
		{
			int size = list.size();

			for (int i = 0; i < size; i++)
			{
				long val = list.get(i) * x;

				if (val < 0 || !perfectSquare(val))
					return false;
			}
