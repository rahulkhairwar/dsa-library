...
//		BufferedReader in;
		...
		void solve() throws IOException
		{
			...
			int maxInd = 0;
			double maxDist = zero.distance(pts[0]);
			Point a = zero;
			Point b, c;

			for (int i = 1; i < n; i++)
			{
				double dist = zero.distance(pts[i]);
			...
			b = new Point(0, pts[maxInd].y + pts[maxInd].x);
			c = new Point(pts[maxInd].x + pts[maxInd].y, 0);
			out.println(CMath.min((long) a.distance(b), (long) a.distance(c), (long) b.distance(c)));
		}
