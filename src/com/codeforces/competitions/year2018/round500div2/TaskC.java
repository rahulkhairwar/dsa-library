package com.codeforces.competitions.year2018.round500div2;
...
public class TaskC
{
	...
	static class Solver implements Runnable
	{
		...
		void solve() throws IOException
		{
			...
			long minArea = getArea(treeMap, n - 1);

			for (int i = n; i < size; i++)
			{
				remove(treeMap, pts[i]);
				add(treeMap, pts[i - n]);
				minArea = Math.min(minArea, getArea(treeMap, i));
			}

			out.println(minArea);
		}

		long getArea(TreeMap<Integer, Integer> map, int ind)
		{
			return (long) (map.lastKey() - map.firstKey()) * (pts[ind] - pts[ind - n + 1]);
		}
