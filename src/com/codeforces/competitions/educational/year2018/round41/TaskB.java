...
//		BufferedReader in;
		...
		void solve() throws IOException
		{
			...
			int[] sleepCum = new int[n];
			int[] cum = new int[n];

			sleepCum[0] = (sleep[0] == 0 ? 0 : lec[0]);
			cum[0] = lec[0];

			for (int i = 1; i < n; i++)
			{
				sleepCum[i] = sleepCum[i - 1] + (sleep[i] == 0 ? 0 : lec[i]);
				cum[i] = cum[i - 1] + lec[i];
			}
			...
			{
				int tot = cum[i] - (i > k ? cum[j] : 0) + sleepCum[n - 1] - (sleepCum[i] - (i > k ? sleepCum[j] : 0));
