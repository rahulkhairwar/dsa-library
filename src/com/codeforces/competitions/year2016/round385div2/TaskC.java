package com.codeforces.competitions.year2016.round385div2;
...
public final class TaskC
{
    ...
    static class Solver
    {
        ...
        void solve()
		{
			...
			for (int i = 0; i < n; i++)
			{
				if (nodes[i].isCap && nodes[i].size > maxSize)
				{
					max = i;
			...
			for (int i = 0; i < n; i++)
			{
				if (!nodes[i].parent.isCap)
					union(nodes[i], nodes[max]);
			}
			...
			for (int x : set)
				cnt += nodes[x].size * (nodes[x].size - 1) / 2;
		...
		void union(Node one, Node two)
		{
			...
			if (parentOne.isCap || parentTwo.isCap)
				parentOne.isCap = parentTwo.isCap = true;
