...
class Foxlings
{
	...
	static class Solver
	{
		...
		void solve(int testNumber)
		{
			...
			Iterator<Map.Entry<Integer, Node>> iterator = disjointSets.entrySet().iterator();

			while (iterator.hasNext())
			{
				Map.Entry<Integer, Node> entry = iterator.next();
				Node parent = findParent(entry.getKey());
				Node value = entry.getValue();

				if (map.containsKey(parent.key))
					map.put(parent.key, Math.max(map.get(parent.key), value.size));
				else
					map.put(parent.key, value.size);
			}
		...
		private void getRelations()
		{
			...
			for (int i = 0; i < m; i++)
			{
				...
				if (!disjointSets.containsKey(from))
					makeSet(from);

				if (!disjointSets.containsKey(to))
					makeSet(to);
