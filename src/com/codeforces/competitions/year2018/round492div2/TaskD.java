...
public class TaskD
{
	...
	static class Solver implements Runnable
	{
		...
		void solve() throws IOException
		{
			...
			out.println(Math.min(cnt, find(arr)));
		}
		...
		int find(int[] arr)
		{
			...
			while (ptr < sz - 1)
			{
				...
				int ind = findIndex(arr, arr[ptr], ptr + 1);

				for (int i = ind; arr[ptr] != arr[ptr + 1]; i--)
				{
					int temp = arr[i];
		...
		int findIndex(int[] arr, int el, int ind)
		{
