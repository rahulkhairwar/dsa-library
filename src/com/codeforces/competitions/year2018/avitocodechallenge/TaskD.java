...
//		BufferedReader in;
		...
		void solve() throws IOException
		{
			...
			out.println(find(0, 0, k));
		}

		long find(int ind, int start, int rem)
		{
			if (ind == n - 1)
			{
				if (rem > 0)
			...
			if (dp[ind][start][rem] != -1)
				return dp[ind][start][rem];
			...
			long max = find(ind + 1, start, rem);

			if (rem > 0)
				max = Math.max(max, sum & find(ind + 1, ind + 1, rem - 1));

			return dp[ind][start][rem] = max;
		}
