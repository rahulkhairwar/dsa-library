    ...
    static class Solver
    {
		...
		void solve()
		{
			...
			for (int i = n - 1; i >= 0; i--)
			{
				...
				if (curr > 0 && valid[curr - 1])
					union(curr, curr - 1);

				if (curr < n - 1 && valid[curr + 1])
					union(curr, curr + 1);
		...
		void union(int one, int two)
		{
			...
			parentOne = findParent(nodes[one]);
			parentTwo = findParent(nodes[two]);
			...
			max = Math.max(max, Math.max(parentOne.sum, parentTwo.sum));
		}
