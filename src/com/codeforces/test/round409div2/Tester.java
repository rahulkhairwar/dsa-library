package com.codeforces.test.round409div2;
	...
	public static void main(String[] args)
	{
		...
		List<TestCase> testCases = readTestCases(
				"/Users/rahulkhairwar/Documents/IntelliJ IDEA Workspace/Competitive "
						+ "Programming/src/com/codeforces/test/round409div2/A.xml");
		...
		for (int i = 0; i < testCases.size(); i++)
		{
			...
			try
			{
				...
				boolean isMatching = testCase.getOutput().equals(outputStream.getOutput().toString());
				...
				System.out.print("Input :\n" + testCase.getInput() + "Output :\n" + outputStream.getOutput()
						+ "Expected Output :\n" + testCase.getOutput() + "Result : " + (isMatching ? "SUCCESS"
																								   : "WRONG "
																								+ "ANSWER") + "\nTime "
						+ "taken " + ": " + (System.currentTimeMillis() - start) + "ms\n");
			}
			catch (IOException e)
			{
				e.printStackTrace();
		...
		for (int i = 0; i < results.size(); i++)
			System.out.println("TestCase #" + (i + 1) + ": " + (results.get(i) ? "SUCCESS" : "WRONG ANSWER"));
	}
