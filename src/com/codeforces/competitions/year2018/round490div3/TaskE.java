...
public class TaskE
{
	...
	static class Solver implements Runnable
	{
		...
		void solve() throws IOException
		{
			...
			adj = new List[n];
			vis = new boolean[n];
			poss = new boolean[n][n];
			reachableFrom = new List[n];

			for (int i = 0; i < n; i++)
			{
				adj[i] = new ArrayList<>();
				poss[i][i] = true;
				reachableFrom[i] = new ArrayList<>();
			}
			...
			Set<Integer>[] reach = new Set[n];
			TreeSet<Integer> set = new TreeSet<>(new Comparator<Integer>()
			{
				@Override public int compare(Integer o1, Integer o2)
				{
					if (reach[o1].size() == reach[o2].size())
						return Integer.compare(o1, o2);

					return Integer.compare(reach[o1].size(), reach[o2].size());
				}
			});
			...
			out.println(ans + unreachable.size());
		}

		void dfs(int node, boolean[] p)
		{
			vis[node] = true;
			p[node] = true;
