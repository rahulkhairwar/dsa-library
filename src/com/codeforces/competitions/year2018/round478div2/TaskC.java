package com.codeforces.competitions.year2018.round478div2;
...
//			System.out.println("st : " + Arrays.toString(strengths));
...
//				System.out.println("arr : " + arrows);
				...
				{
					int pos = binarySearch(arrows);

					if (pos == -1)
						pos = 0;

					long cnt = strengths[pos];
...
//					out.println(n - pos);
		...
		int binarySearch(long arrows)
		{
			...
			{
				int mid = low + high >> 1;
				long midCnt = strengths[mid];

				if (midCnt == arrows)
					return mid + 1;
				else if (midCnt < arrows)
				{
					if (mid == n - 1 || strengths[mid + 1] > arrows)
						return mid + 1;
